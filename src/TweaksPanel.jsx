// Tweaks panel — floating design controls. Exported as reusable components.
...
export function useTweaks(defaults) {
  const [values, setValues] = React.useState(() => {
    const saved = localStorage.getItem('projektllm_tweaks');
  ...
  const setTweak = React.useCallback((keyOrEdits, val) => {
    ...
    setValues((prev) => {
      ...
      localStorage.setItem('projektllm_tweaks', JSON.stringify(next));
      return next;
    });
  }, []);
...
export function TweaksPanel({ title = 'Tweaks', children }) {
  ...
  // Listen for Ctrl+Shift+T to toggle panel
  React.useEffect(() => {
    const handler = (e) => {
